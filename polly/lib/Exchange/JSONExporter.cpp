...
static cl::opt<std::string>
    ExportPrefix("polly-export-jscop-prefix",
        cl::desc("Prefix to append to the export .jscop files"),
...
struct JSONExporter : public ScopPass {
  static char ID;
  std::string CustomFilePrefix;
  explicit JSONExporter() : ScopPass(ID), CustomFilePrefix("") {}
  explicit JSONExporter(const std::string &C)
  : ScopPass(ID), CustomFilePrefix(C) {}
...
static std::string getFileName(Scop &S, StringRef Suffix = "") {
  ...
  std::string FileName = ExportPrefix + FunctionName + "___" + S.getNameStr() + ".jscop";
...
static json::Value getJSON(Scop &S) {
  ...
  root["scheduleTree"] = S.getScheduleTree().to_str();
  if (LineBegin != (unsigned)-1)
...
static void exportScop(Scop &S, std::string customFileName = "") {

  std::string FileName = ImportDir + "/" + customFileName + getFileName(S);
...
Pass *polly::createJSONExporterPass(const std::string &c) { return new JSONExporter(c); }
