...
enum OptimizerChoice { OPTIMIZER_NONE, OPTIMIZER_ISL, OPTIMIZER_HM_RACETRACK, OPTIMIZER_HM_SCHEDULE_SEARCH, OPTIMIZER_HM_PATTERN };
...
static cl::opt<OptimizerChoice>
    Optimizer("polly-optimizer", cl::desc("Select the scheduling optimizer"),
              cl::values(clEnumValN(OPTIMIZER_NONE, "none", "No optimizer"),
                         ...
                         clEnumValN(OPTIMIZER_HM_RACETRACK, "hm-rtm",
                                    "The custom racetrack memory optimizer"),
                         clEnumValN(OPTIMIZER_HM_SCHEDULE_SEARCH, "hm-schedule-search",
                                    "The custom schedule search optimizer"),
                         clEnumValN(OPTIMIZER_HM_PATTERN, "hm-pattern", "The pattern match schedule optimizer")),
...
static cl::opt<std::string> KernelName(
    "polly-hm-kernel-base-name",
...
void initializePollyPasses(PassRegistry &Registry) {
  ...
  initializeHMScheduleModifierPass(Registry);
  initializeHMRacetrackScheduleOptimizerPass(Registry);
  initializeHMRacetrackScheduleSpaceSearcherPass(Registry);
  initializeHMRacetrackSchedulePatternMatcherPass(Registry);
}
...
void registerPollyPasses(llvm::legacy::PassManagerBase &PM) {
  ...
  if (ExportJScop)
    PM.add(polly::createJSONExporterPass(KernelName + "__before-opt__"));
  ...
  if (Target == TARGET_CPU || Target == TARGET_HYBRID)
    ...
    case OPTIMIZER_ISL:
      ...
      break;
    case OPTIMIZER_HM_RACETRACK:
      PM.add(polly::createHMRacetrackScheduleOptimizerPass());
      break;
    case OPTIMIZER_HM_SCHEDULE_SEARCH:
      PM.add(polly::createHMRacetrackScheduleSpaceSearcherPass());
      break;
    case OPTIMIZER_HM_PATTERN:
      PM.add(polly::createHMRacetrackSchedulePatternMatcherPass());
      break;
    }


  if (ExportJScop)
    PM.add(polly::createJSONExporterPass(KernelName + "__after-opt__"));
