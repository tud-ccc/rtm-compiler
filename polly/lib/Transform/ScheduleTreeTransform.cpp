...
struct ExtensionNodeRewriter
  ...
  isl::schedule visitExtension(const isl::schedule_node &Extension,
    ...
    // Only unite with other extensions if the dimensionality is not zero...
    //... in that case, this extension is directly below the root, and we are
    // done.
    //std::cout << ExtDomain.dim(isl::dim::in);
    // clearly, this cannot work as ExtDomain is a union set. Why did I
    // ever introduce that code?
//    if(ExtDomain.dim(isl::dim::in) > 0) {
//      Extensions = ChildExtensions.unite(ExtDomain);
//    } else {
